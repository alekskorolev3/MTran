    ...
    if (run):
        ...
        for i, token in enumerate(set_of_tokens):
            ...
            if token[2] == "{":
                ...
                if len(_brackets) == 0:
                    raise SyntaxError(
                        str(token[0] + 1) + ":" + str(token[1]) + " Error: Unmatched closing bracket at token: '" + str(
                            token[2]) + "'")
                else:
                    _brackets.pop()
            ...
            if token[2] in tokens.keywords:
                if set_of_tokens[i - 1][2] not in tokens.datatypes:
                    ...
                    raise SyntaxError(
                        str(token[0] + 1) + ":" + str(token[1]) + " Error: Expected unqualified-id before '" + str(
                            token[2]) + "'")
                ...
                if token[2] not in _ident:
                    if set_of_tokens[i + 1][2] == "(":
                        ...
                        identifiers_table.add_row(
                            [token[2], "Identifier of " + str(tokens.datatypes[set_of_tokens[i - 1][2]])])
                        continue
                    else:
            ...
            if token[2] in tokens.operators:
                if token[2] == '++':
                    if set_of_tokens[i - 1][2] in _ident or set_of_tokens[i + 1][2] in _ident:
                        ...
                        raise SyntaxError(
                            str(token[0] + 1) + ":" + str(token[1]) + " Expected identifier before '" + str(
                                token[2]) + "' token")
        ...
        if len(_brackets) != 0:
            raise SyntaxError(
                str(_brackets[-1][0] + 1) + ":" + str(
                    _brackets[-1][1]) + " Error: Unmatched opening bracket at token: '" + str(
                    _brackets[-1][2]) + "'")
